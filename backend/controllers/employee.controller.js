...
const getEmployees = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message:"something went wrong while getting all the employees", error:error.message
    })
  }
};
...
const getSingleEmployee = async(req,res)  =>{
  try {
    ...
    const employee = Employee.findById(employeeId).populate(["company", "department", "designation","salaryStructure","documents"])

    if(!employee){
      return res.status(404).json({message:"employee not found with given Id"})
    }
    return res.status(200).json({message:"employee fetched"})
  } catch (error) {
    return res.status(500).json({message:"employee not fetched", error:error.message})
  }
}
...
const updateEmployee = async(req,res) =>{
  try {
    ...
    return res.status(500).json({message:"employee not updated", error:error.message})
  }
}
...
const updateEmployeeStatus = async(req,res) =>{
  try {
    ...
    const employee = await Employee.findByIdAndUpdate(employeeId,{isActive},{new:true})
...
const deleteEmployee = async(req,res)=>{
  try {
    ...
    return res.status(500).json({message:"employee not deleted", error:error.message})
  }
}
